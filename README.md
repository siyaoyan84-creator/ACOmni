...
4. MER2024 parsing/metrics and paper mechanism analyses.
...
analysis/             cue coverage, error transition, cue-gain association
splits/mer2024/       public Balanced50 IDs and deterministic split builder
...
`splits/mer2024/balanced50_ids.txt` contains the 300 real sample IDs used by
Balanced50 (50 per class). The label mapping is public in
`class_mapping.json`.

The Fresh-test300 split uses seed `20260722`, 50 samples per class, excludes
Balanced50, and is generated by:

```bash
python splits/mer2024/build_splits.py \
  --labels-csv /path/to/MER2024/label-disdim.csv \
  --video-root /path/to/MER2024/videos \
  --used-test-jsonl /path/to/balanced50.jsonl \
  --output-root /path/to/generated-splits
```

The actual `fresh_test300_ids.txt` is not yet included because the generated
source file is not present in the release workspace. It must be added from the
frozen paper split before public release; IDs will not be regenerated or
invented.
...
Table V cue coverage:

```bash
python analysis/affective_cue_coverage.py --predictions predictions.jsonl
```

Table VI transition analysis expects CSV columns
`sample_id,baseline_correct,acor_correct`:

```bash
python analysis/error_transition.py --input transitions.csv
```

Cue-gain association additionally expects
`baseline_cue_types,acor_cue_types`:

```bash
python analysis/cue_gain_association.py --input cue_gain.csv
```

The paper's ACOR-A.R. transition counts are Corrected 11, New Error 5,
Both Right 109, Both Wrong 8, and Net Gain +6.
...
- Keep this GitHub repository private until the frozen Fresh-test300 IDs,
  final citation, and public adapter release have passed review.
