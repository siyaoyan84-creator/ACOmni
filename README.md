...
4. MER2024 parsing, split construction, and metrics.
...
The frozen Fresh-test300 input used by the paper is:

```text
eval_results/mer2024_acor_eval/mer2024_fresh_test50_v2_neutral.jsonl
```

The split uses seed `20260722`, 50 samples per class, excludes Balanced50,
and is generated by:
...
The actual `fresh_test300_ids.txt` is not yet included because the frozen JSONL
above is not present in this local release workspace. It must be extracted from
that exact file before public release; IDs will not be regenerated or invented.
