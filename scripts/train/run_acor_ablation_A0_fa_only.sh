...
# ACOR matched reward ablation A0: format + accuracy only.
...
echo "=========================================="
echo "ACOR Reward Ablation A0 - Format + Accuracy Only"
echo "Start checkpoint: SFT checkpoint-2264"
echo "Reward funcs: format,accuracy"
...
# ========== Reward Configuration - A0 ==========
export GRPO_REWARD_FUNCS="format,accuracy"

# ========== Affective/Emotion Rewards Disabled ==========
...
# ========== Output Directory ==========
export OUTPUT_DIR="${OUTPUT_ROOT}/acor_reward_ablation_A0_fmt_acc_only_sft2264_500"
...
torchrun --nproc_per_node $NPROC_PER_NODE --nnodes=$NNODES --node_rank=$NODE_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT \
    ${PROJECT_ROOT}/${PROJECT_ROOT}/src/open_r1/grpo_qwenomni_acor_er.py \
    ...
    --num_train_epochs 1 \
    --run_name acor_reward_ablation_A0_fmt_acc_only_sft2264_500 \
