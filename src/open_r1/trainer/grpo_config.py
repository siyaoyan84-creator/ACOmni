...
class GRPOConfig(TrainingArguments):
    ...
    def __post_init__(self):
        ...
        # [GRPO_DDP_FIX] Set ddp_find_unused_parameters to False for DDP training
        # This prevents "mark ready twice" errors with LoRA parameters
        import os
        if os.environ.get("GRPO_DDP_FIND_UNUSED_PARAMETERS"):
            self.ddp_find_unused_parameters = os.environ.get("GRPO_DDP_FIND_UNUSED_PARAMETERS").lower() == "true"
        else:
            # Default to False for GRPO to avoid DDP issues
            self.ddp_find_unused_parameters = False

        # [GRPO_DDP_FIX] Set gradient_checkpointing_kwargs to use non-reentrant checkpointing
        # This is required for DDP with LoRA to avoid "mark ready twice" errors
        if self.gradient_checkpointing:
            if not hasattr(self, 'gradient_checkpointing_kwargs') or self.gradient_checkpointing_kwargs is None:
                self.gradient_checkpointing_kwargs = {"use_reentrant": False}
            elif isinstance(self.gradient_checkpointing_kwargs, dict):
                self.gradient_checkpointing_kwargs["use_reentrant"] = False